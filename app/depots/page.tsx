...
export default function DepotsPage() {
  const [items, setItems] = useState<storage.DepotItem[]>([]);
  ...
  function startEdit(it: storage.DepotItem) {
    setEditingId(it.id);
    setForm({
      name: it.name || "",
      phone: it.phone || "",
  ...
  function save() {
    ...
    try {
      ...
      reload();
      resetForm();
      setMsg("✅ Deposito aggiunto.");
    } catch (e: any) {
      if (String(e?.message) === "LIMIT_100") {
        alert("Limite massimo depositi raggiunto: 100");
      } else {
  ...
  async function exportPdf() {
    ...
    await storage.exportPdf({
      filename: "depositi.pdf",
      title: "Elenco Depositi",
  ...
  return (
    <div style={{ padding: 16, maxWidth: 860, margin: "0 auto" }}>
      <h1 style={{ fontSize: 28, marginBottom: 6 }}>🏭 Gestione Depositi</h1>
      <div style={{ opacity: 0.85, fontWeight: 700 }}>
        Inserisci depositi con contatti e posizione GPS. (Max 100)
      </div>
