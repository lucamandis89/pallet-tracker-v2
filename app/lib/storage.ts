...
function buildGenericCsv(headers: string[], rows: Array<Record<string, unknown>> | Array<Array<unknown>>): string {
  const head = headers.map(escapeCsvCell).join(",");
  const lines: string[] = [head];
  for (const r of rows as any[]) {
    if (Array.isArray(r)) {
      lines.push((r as Array<unknown>).map(escapeCsvCell).join(","));
    } else {
...
export function downloadCsv(...args: any[]): void {
  ...
  // Usage A (legacy): downloadCsv(items?, filename?)
  // Usage B (generic): downloadCsv(filename, headers, rows)
  if (typeof args[0] === "string") {
    const filename: string = args[0];
    const headers: string[] = Array.isArray(args[1]) ? args[1] : [];
    const rows: any[] = Array.isArray(args[2]) ? args[2] : [];
    const csv = buildGenericCsv(headers, rows);
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
    triggerDownload(blob, filename);
    return;
  }

  const items: HistoryItem[] = Array.isArray(args[0]) ? args[0] : getHistory();
  const filename: string = typeof args[1] === "string" ? args[1] : "history.csv";
...
export function addDriver(
  input: string | { name: string; phone?: string; address?: string; note?: string; notes?: string; lat?: number; lng?: number }
): DriverItem {
  const obj =
    typeof input === "string"
      ? { name: input }
      : {
          ...
          lat: (input as any)?.lat,
          lng: (input as any)?.lng,
        };
